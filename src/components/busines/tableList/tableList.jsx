  ...
  render() {
    ...
    const tableProps = {
      ...
      props: {
        ...
        scroll: { x: isScrollWidthStatus ? 10 : 0, y: tableHeight ? tableHeight : 0 } //&& tableDatas.length 
      },
      attrs: {},
      on: {
  ...
  methods: {
      ...
      this.tableColumns.forEach(item => {
        if (item.children && item.children.length) {
          _.forEach(item.children, subItem => {
            width = width + this.getColumnsWidth(subItem);
          });
        } else {
      ...
      const recursionColumns = (res) => {
        let _column = res.map(item => {
          if ((item.hidden == undefined || !item.hidden) && (item.sys_hidden == undefined || !item.sys_hidden)) {
            ...
            if (item.children && item.children.length) {
              _item.children = recursionColumns(item.children)
            }
    ...
    handleFiltersHeaderDatas(res, type) {
      if (this.filtersHeaderColumns) {
        ...
        this.tableColumns = recursionFilterTableColumns(this.tableColumns);
      }
    },
    //获取列表表头数据
    ...
    handleTableChange(pagination, filters, sorter) {
      ...
      if (!this.lockingEd) {
        if (this.options.pageUrl != "") {
          ...
          if (!_.isEmpty(sorter) && sorter.columnKey != "") {
            if (sorter.column) {
              let key = sorter.column.filter_name && sorter.column.filter_name != "" ? sorter.column.filter_name : sorter.column.ora_name;
    ...
    handleChangeColumns(list) {
      ...
      if (this.filtersHeader) {
        this.setFiltersHeaderColumns(this.tableColumns);
        this.handleFiltersHeaderDatas(this.tableDatas, true);
      }
    },
    //还原
    operateRestore() {
    ...
    async getFilterDict() {
      ...
      //获取关系符字典
      this.relationLists = [{ "title": "并且", "key": "1" }, { "title": "或者", "key": "2" }];
    },
